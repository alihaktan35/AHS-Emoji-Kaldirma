...
function removeEmojis() {
 ...
 const processedLink = processLink(removedEmojis);

 const outputLink = document.getElementById('outputLink');

 outputLink.innerHTML = `<a href="${processedLink}" target="_blank">${processedLink}</a>`;
...
function processLink(link) {
    ...
    if (httpIndex === 0 || httpsIndex === 0) {
        link = link.slice(httpIndex.length + 7);
    }
